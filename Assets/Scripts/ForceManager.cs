...
public class ForceManager : MonoBehaviour
{
    ...
    private const float LOWERLIMIT = 1.05f;  //not drop below this limit
    ...
    void Update()
    {
        ...
        //always make the cube facing front
        ...
        */
        ...
        //old formula
        //thumb: vout = 0.5094 * force(g) + 419.75
        //palm: vout = 0.0416 * force(g) + 520.72
        //new formula
        ...
        if (isGrabbed)
        {
            ...
            /*
            if (Pb != null) {
                Pb.BarValue = fsr / targetForce * 50;  //so that the green bar show when it hit the middle
            ...
            */
            ...
            if (fsr < targetForce)            {
                ...
                else
                {
                    ...
                    if (GameConfiguration.CDRatio)  // if CD ratio
                    {
                        ...
                        if ((hv.oriHandPosition.y - newHandPosition.y) > 0.01)  //threshold
                        {
                            //drop the object
                            gb.EndTransform();
                        }
            ...
            {               
                ...
                {
                    yOffset *= 6;  //accelerate by offset with visible quantum                                   
                }
                else
                {
                ...
                if (GameConfiguration.CDRatio)  //if CD Ratio
                {
                    ...
                    if ((newHandPosition.y - hv.oriHandPosition.y) > 0.15)  //threshold
                    {
                        newHandPosition = new Vector3(hv.oriHandPosition.x + xOffset, hv.oriHandPosition.y + 0.15f, hv.oriHandPosition.z + zOffset);
                        if (GameConfiguration.mode == 'M')
                        {
                            //apply offset so the object is at the middle of the palm
                            newObjectPosition = new Vector3(indexposition.x + xOffset, indexposition.y + 0.15f + offsetFromOriHand - 0.02f, indexposition.z + zOffset);
                        }
                        else
                        {
                            newObjectPosition = new Vector3(indexposition.x + xOffset, indexposition.y + 0.15f + offsetFromOriHand, indexposition.z + zOffset);
                        }
                    }
                    else{
                        if (GameConfiguration.mode == 'M')
